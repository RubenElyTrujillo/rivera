        ...
        <div className="space-y-4">
          {materials.map((mat, idx) => (
            <FormCard key={mat.id}>
              ...
              {/* Acabados — solo si el material ya tiene ID real en BD */}
              {mat.id > 1000000 ? (
                ...
                <div className="mt-4 border-t border-[hsl(0,0%,90%)] pt-4">
                  ...
                  {expandedFinishes[mat.id] && (
                    <div className="mt-3 space-y-2">
                      {mat.finishes?.map((f) => (
                        <div key={f.id} className="flex items-center gap-3 bg-[hsl(0,0%,97%)] px-4 py-3 rounded text-sm">
                          {f.image && <img src={f.image} alt={f.name} className="w-10 h-10 object-cover rounded shrink-0" />}
                          <div className="flex-1 min-w-0">
                            <p className="font-semibold truncate">{f.name}</p>
                            <p className="text-[hsl(0,0%,50%)] font-mono text-xs">{f.code}{f.collection ? ` · ${f.collection}` : ""}{f.dims ? ` · ${f.dims}` : ""}</p>
                          </div>
                          <button onClick={() => deleteFinish(mat.id, f.id)} className="text-red-400 hover:text-red-600 shrink-0">
                            <Trash2 size={14} />
                          </button>
                        </div>
                      ))}
